...
export function EmptyState({
  ...
  return (
    <section className={cn('rounded-[1.75rem] border border-dashed border-current/15 bg-current/[0.03] p-8 text-center', className)}>
...
export function TaskEmptyState({ taskLabel = 'listings', className }: { taskLabel?: string; className?: string }) {
  return (
    <EmptyState
      ...
      description={`New ${taskLabel} will appear here automatically as soon as they go live. The layout stays ready even when the directory is quiet.`}
